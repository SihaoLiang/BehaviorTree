...
namespace BehaviorTreeDebuger
{
    ...
    /// Editor helper class.
    /// </summary>
    public static class XUnityEditorUtility
    {
        ...
        public static object[] GetCustomAttributes(Type type)
        {
            ...
            if (!XUnityEditorUtility.typeAttributeCache.TryGetValue(type, out customAttributes))
            {
                customAttributes = type.GetCustomAttributes(true);
                XUnityEditorUtility.typeAttributeCache.Add(type, customAttributes);
            }
        ...
        {
            object[] customAttributes;
            if (!XUnityEditorUtility.fieldAttributeCache.TryGetValue(field, out customAttributes))
            {
                customAttributes = field.GetCustomAttributes(true);
                XUnityEditorUtility.fieldAttributeCache.Add(field, customAttributes);
            }
            return customAttributes;
        }
        ...
        {
            object[] objArray = XUnityEditorUtility.GetCustomAttributes(field);
        ...
        {
            object[] objArray = XUnityEditorUtility.GetCustomAttributes(type);
            for (int i = 0; i < (int)objArray.Length; i++)
            {
                if (objArray[i].GetType() == typeof(T) || objArray[i].GetType().IsSubclassOf(typeof(T)))
                {
